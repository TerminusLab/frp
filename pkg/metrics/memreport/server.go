...
type serverMetrics struct {
	...
	mu   sync.Mutex
	// proxy name -> login user for per-user traffic when using 0.68 metrics.ServerMetrics signatures.
...
func (m *serverMetrics) NewProxy(name string, proxyType string, user string, _ string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.proxyUsers[name] = user
...
func (m *serverMetrics) CloseProxy(name string, proxyType string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	user := m.proxyUsers[name]
	delete(m.proxyUsers, name)
